<div>
    <div class="bg-gray-100 dark:bg-gray-900 min-h-screen py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- 左右布局容器 -->
            <div class="{{ $link->message && $link->message !== 'None' ? 'flex flex-col lg:flex-row gap-6' : 'flex justify-center' }}">
                <!-- 左侧名片信息 -->
                <div class="lg:w-1/3 lg:min-w-[320px]">
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden {{ $link->message && $link->message !== 'None' ? 'lg:sticky lg:top-6' : '' }}">
                        <!-- 频道/群组头部信息 -->
                        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                            ...
                            @if($link->introduction && $link->introduction !== 'None')
                            <div class="mt-4 text-gray-600 dark:text-gray-300 text-sm">
                                {!! nl2br(e($link->introduction)) !!}
                            </div>
                            @endif

                            <div class="mt-4 flex justify-center space-x-6">
                                <div class="text-center">
                                    <div class="text-xl font-semibold text-gray-900 dark:text-white">{{ number_format($link->member_count) }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">订阅者</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-xl font-semibold text-gray-900 dark:text-white">{{ number_format($link->view_count) }}</div>
                            ...
                            <div class="flex">
                                ...
                                <div class="flex-grow">
                                    <div class="flex items-center mb-2">
                                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $link->name }}</h2>
                                        <span class="ml-2 text-sm text-gray-500 dark:text-gray-400">
                                            {{ $link->created_at->format('Y-m-d H:i') }}
                                        </span>
                                    </div>
                                    <div class="prose dark:prose-invert max-w-none">
                                        <p class="text-gray-700 dark:text-gray-300 whitespace-pre-line">{!! nl2br(e($link->message)) !!}</p>
                                    </div>
                                    <div class="mt-4 flex items-center text-sm text-gray-500 dark:text-gray-400">
                                        ...
                                        {{ number_format($link->view_count) }} 次浏览
                                    </div>
                                </div>
                            </div>
                        </div>
