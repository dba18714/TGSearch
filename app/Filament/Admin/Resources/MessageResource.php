        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('chat.name')
                    ...
                    ->action(
                        Tables\Actions\Action::make('viewChat')
                            ->label(fn(Message $record) => "所有者详情 #{$record->chat_id}")
                            ->slideOver()
                            ->modalWidth('lg')
                            ->form(fn(Form $form) => ChatResource::form($form)->getComponents())
                            ->fillForm(fn(Message $record) => $record->chat->toArray())
                            ->action(function (array $data, Message $record) {
                                $record->chat->update($data);
                                Notification::make()
                                    ->title('所有者信息已更新')
                                    ->success()
                                    ->send();
